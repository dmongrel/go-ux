...
- `editors/` — embeddable text-editor-with-tabs component (novel-writing / prose-editing focus, plus an AI-diff-review API surface). Current: `Group` (parent, up to 4 `Pane`s via one level of nested `container.Split`), hand-rolled tab bar, real editable Document-backed content (synced across split panes), live-persisted layout (`db.EditorPane`/`EditorTab`, a new relational domain on `db.DB`). No file I/O, line numbers/soft wrap, markdown preview, diff view, font settings, or file watching yet. See `editors.md`.
