...
- `editors/` — embeddable text-editor-with-tabs component (novel-writing / prose-editing focus, plus an AI-diff-review API surface). Current: `Group` (parent, up to 4 `Pane`s via one level of nested `container.Split`), hand-rolled tab bar, real editable Document-backed content with line numbers/soft-wrap toggle (synced across split panes), Ctrl+scroll per-Group font sizing (via `fontsettings`, persisted), Ctrl+S save-to-disk (`Group.SaveTab`), goldmark-based Markdown preview toggle (snapshot render, not live), `Group.OpenFile`/`ProposeDiff` mcp_tooling API (go-difflib-based diff review, Accept writes to disk), one-`fsnotify.Watcher`-per-`Group` file watching (auto-reload or south-bar notify, per a persisted `file_watch_mode` setting), per-`groupID` settings node (`editors.RegisterSettings`/`ApplyEditorSettings`), live-persisted layout (`db.EditorPane`/`EditorTab`, a new relational domain on `db.DB`). Original Phase 2 plan fully implemented; see `editors.md` for what's still deferred.
- `fontsettings/` — shared Ctrl+scroll-adjustable font configuration (`FontSettings`, clamping, a per-instance listener-registry `State` type, `DetectMonospaceFonts` (Windows), `SeedFontProperties`/`ReadFontProperties`). Extracted from `terminal`'s original package-global font-settings mechanism so `editors` could reuse it with independent (not globally shared) state per `Group`. `terminal` still owns its own font *rendering* (loading font bytes for its custom canvas rasterizer) — only the settings/clamping/listener plumbing moved.
