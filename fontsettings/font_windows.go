...
// sample) — the property that makes a font usable for a fixed-grid
// terminal, or a predictable-width editor gutter/content area. Mirrors
// go-ux/terminal's DetectShells()'s contract: never returns an error, an
// empty result on any enumeration failure is handled gracefully by callers
// (a font-family settings enum then only offers "(default)", the bundled
// font).
...
// recognize (e.g. "Consolas" not "Consolas (TrueType)"). Exported since
// go-ux/terminal's own font-loading code (which stays in terminal — it's
// tied to terminal's custom canvas rasterization, not shared with editors)
// also needs to resolve a chosen family name back to a registry entry.
