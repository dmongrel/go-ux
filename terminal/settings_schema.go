...
// readTerminalSettings looks up the Terminal node's default_shell and
// close_on_exit values. found is false if RegisterSettings has never been
// called against database (no Terminal node yet) — callers should fall back
// to their own defaults in that case rather than treating it as an error.
func readTerminalSettings(database *db.DB) (defaultShell string, closeOnExit bool, found bool, err error) {
	nodes, err := database.ListSettings()
	if err != nil {
	...
	return defaultShell, closeOnExit, true, nil
}
